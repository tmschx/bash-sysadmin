...
# Step 1: update
printf "==== STEP 1. Update: resynchronizing the package index files from their sources specified in /etc/apt/sources.list.\n"
apt-get update -q; ERRORCODE=$?
if [[ ${ERRORCODE} != 0 ]]; then
...
# Step 2: autoclean
printf "==== STEP 2. Autoclean: clearing out the local repository of old package files that can no longer be downloaded.\n"
...
# Step 4: Upgrade
printf "==== STEP 4. Upgrade: installing the newest versions of all packages currently installed  on the system from the sources in /etc/apt/sources.list.\n"
...
# Step 5: Autoremove
printf "==== STEP 5. Autoremove: removing packages that were automatically installed to satisfy dependencies for other packages and that are now no longer needed.\n"
